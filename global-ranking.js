...
function setupMobileV2(){
  ...
  const left=document.querySelector('.leftcol');
  const filters=document.querySelector('.filters');
  const ranking=document.querySelector('.global-rank-card');
  if(!nav||!left||!filters||!ranking)return;
  document.body.dataset.navV2='1';

  const homeBtn=nav.querySelector('[data-view="home"]');
  const contestsBtn=document.getElementById('contestsNav');
  const profileBtn=nav.querySelector('[data-view="profile"]');
  const logoutBtn=document.getElementById('logoutBtn');

  // Dedicated Search drawer: move the existing working filter form, preserving its listeners.
  const searchDrawer=document.createElement('section');
  searchDrawer.id='searchDrawer';searchDrawer.className='drawer hidden';
  searchDrawer.innerHTML='<div class="drawer-inner fs-v2-sheet"><button id="closeSearchV2" class="close">×</button><div class="fs-v2-kicker">RECHERCHE JOUEURS</div><h2>🔎 Trouver un talent</h2><p class="msg">Filtre par code postal, poste et catégorie.</p><div id="searchFormSlot"></div></div>';
  document.body.appendChild(searchDrawer);
  searchDrawer.querySelector('#searchFormSlot').appendChild(filters);
  filters.querySelector('h2')?.remove();

  // Dedicated Ranking drawer: move the complete live ranking card.
  const rankDrawer=document.createElement('section');
  rankDrawer.id='rankDrawer';rankDrawer.className='drawer hidden';
  rankDrawer.innerHTML='<div class="drawer-inner fs-v2-sheet"><button id="closeRankV2" class="close">×</button><div class="fs-v2-kicker">FOOTSHOW LEAGUE</div><h2>🥇 Classement</h2><div id="rankSlot"></div></div>';
  document.body.appendChild(rankDrawer);
  rankDrawer.querySelector('#rankSlot').appendChild(ranking);

  // Compact home hero/actions.
  const hero=document.createElement('div');hero.className='fs-v2-home';
  hero.innerHTML=`<div class="fs-v2-welcome"><div><div class="fs-v2-kicker">FOOTSHOW</div><h1>Montre ton talent.</h1><p>Publie tes actions, relève les défis et grimpe au classement.</p></div><div class="fs-v2-ball">⚽</div></div><div class="fs-v2-actions"><button id="openSearchV2"><span>🔎</span><b>Rechercher</b><small>Trouver un joueur</small></button><button id="openFeedV2"><span>📹</span><b>Feed</b><small>Meilleures actions</small></button><button id="openQuizV2"><span>🧠</span><b>Quiz</b><small>Teste-toi</small></button><button id="openGameV2"><span>🎮</span><b>Arcade</b><small>Jeux 3D</small></button><button id="openMessagesV2"><span>💬</span><b>Messages</b><small>Discussions privées</small></button><button id="openStatsV2"><span>📊</span><b>Mes stats</b><small>Ta progression</small></button></div>`;
  left.prepend(hero);

  // Keep existing nodes/listeners when possible and reduce bottom nav to 5 items.
  ...
  nav.replaceChildren();
  if(homeBtn){homeBtn.innerHTML='⌂<small>Accueil</small>';nav.appendChild(homeBtn)}
  if(contestsBtn){contestsBtn.innerHTML='🏆<small>Concours</small>';nav.appendChild(contestsBtn)}
  nav.appendChild(publishBtn,rankBtn);
  if(profileBtn){profileBtn.innerHTML='👤<small>Profil</small>';nav.appendChild(profileBtn)}

  // Logout belongs in Profile, not in the main nav.
  ...
  const closeAll=()=>document.querySelectorAll('.drawer:not(.hidden)').forEach(x=>x.classList.add('hidden'));
  const openSearch=()=>{searchDrawer.classList.remove('hidden');setTimeout(()=>document.getElementById('filterDepartment')?.focus(),250)};
  const openRank=()=>{rankDrawer.classList.remove('hidden');loadGlobalRanking()};
  document.getElementById('openSearchV2').onclick=openSearch;
  document.getElementById('closeSearchV2').onclick=()=>searchDrawer.classList.add('hidden');
  document.getElementById('closeRankV2').onclick=()=>rankDrawer.classList.add('hidden');
  ...
  document.getElementById('openFeedV2').onclick=()=>document.getElementById('feedOpen')?.click();
  document.getElementById('openQuizV2').onclick=()=>document.getElementById('homeQuizBtn')?.click();
  document.getElementById('openGameV2').onclick=()=>document.getElementById('homeGameBtn')?.click();
  document.getElementById('openMessagesV2').onclick=()=>document.getElementById('messagesNav')?.click();
  document.getElementById('openStatsV2').onclick=()=>document.getElementById('statsOpen')?.click();

  // Hide redundant large home blocks: access remains through the new compact actions.
